...
async function main() {
  ...
  const keepLeague = await makeLeagueFilter();
  ...
  const changed = await saveState(state);
  console.log(
    `\nĐã quét ${events.size} trận (bỏ qua ${summary.skipped} theo bộ lọc giải) · ` +
