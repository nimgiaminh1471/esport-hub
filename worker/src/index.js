...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && (url.pathname === '/note' || url.pathname === '/note.json')) {
      ...
      const snapshot = {
        ...(await noteSnapshot(env, url.searchParams.get('period') || undefined)),
        // Phần chốt theo ngày đi kèm luôn trong cùng một snapshot: trang trên
        // GitHub Pages chỉ gọi được MỘT endpoint, tách ra thành hai lời gọi thì
        // nửa trang này hiện trước nửa kia.
        ngay: await daySnapshot(env, url.searchParams.get('month') || undefined),
      };
